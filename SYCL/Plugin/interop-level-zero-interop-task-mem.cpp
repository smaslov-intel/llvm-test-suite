...
public:
    int operator()(const cl::sycl::device &dev) const override {
        return (dev.get_platform().get_backend() == cl::sycl::backend::level_zero) ? 1 : 0;
    }
};
...
int main() {
    ...
    ze_context_handle_t ze_context = sycl_queue.get_context().get_native<sycl::backend::level_zero>();
    std::cout << "zeContextGetStatus = " << zeContextGetStatus(ze_context) << std::endl;
    ...
    sycl_queue.submit([&](cl::sycl::handler &cgh) {
        ...
        cgh.interop_task([&](const cl::sycl::interop_handler &ih) {
            ...
            zeMemGetAllocProperties(ze_context, device_ptr, &memAllocProperties, nullptr);
